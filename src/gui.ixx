...
void gui::start_scan() {
    ...
    int count = std::max(1, atoi(buf[0] ? buf : "1"));
    ...
    // Pre-create callbacks passed into the scanner worker.
    ...
    // Re-enable controls when the scan naturally stops.
  ...
  //  std::future<void> scan_future = std::async(std::launch::async, [&]() {
    ...
    // Alternate direct call retained for reference while scan execution stays threaded.
  //  scanner::run_scan(count, ports_str, threads, delay, random_mode, proxy, path,
...
LRESULT CALLBACK gui::WndProc(HWND h, UINT m, WPARAM w, LPARAM l) {
    HINSTANCE hinst = reinterpret_cast<HINSTANCE>(GetWindowLongPtrA(h, GWLP_HINSTANCE));
